import {
  existsSync, mkdirSync, watchFile, unwatchFile, Stats, rmSync,
  promises as fs
} from 'node:fs';
...
class SimpleDB extends EventEmitter {
  ...
  private async migrateLegacyCollection(name: string, legacyPath: string, newPath: string): Promise<void> {
    try {
...
class Collection extends EventEmitter {
  ...
  private loadAllShardsSync(): void {
    try {
      ...
      for (const file of shardFiles) {
        const shardId = parseInt(file.replace(/^shard_(\d+)\.json$/, '$1'));
        this.loadShardSync(shardId);
      }
  ...
  private loadShardSync(shardId: number): void {
  ...
  private async saveShardAsync(shardId: number): Promise<void> {
  ...
  private saveShardSync(shardId: number): void {
    const shardInfo = this.shards.get(shardId);
    const shardDataMap = this.shardData.get(shardId);
    if (!shardInfo || !shardDataMap) return;
  ...
  private queueSaveOperation(): void {
    ...
    this.saveTimer = setTimeout(async () => {
      ...
      try {
        // Parallel save with concurrency control
        const limit = pLimit(this.concurrency);
        await Promise.all(
          Array.from(dirtyShards).map(shardId =>
            limit(() => this.saveShardAsync(shardId))
          ));
        await this.saveMetadata();
      } catch (error) {
        console.error('Save error:', error);
        // Re-add dirty shards on failure
        dirtyShards.forEach(shardId => this.dirtyShards.add(shardId));
      } finally {
        this.saving = false;

        // Process any new dirty shards
  ...
  private async reloadShard(shardId: number): Promise<void> {
    const shardDataMap = this.shardData.get(shardId);
    ...
    shardDataMap.clear();
    await this.loadShardSync(shardId);
  ...
  findOne(query: any = {}): any {
    ...
    if (queryKeys.length === 1) {
      const [field, value] = Object.entries(query)[0];
      if (typeof value !== 'object' || value === null) {
        const results = this.findByIndex(field, value);
        return results.length ? results[0] : null;
      }
    }

    for (const doc of this.data.values()) {
      if (this.matchDocument(doc, query)) return doc;
    }
    return null;
  }
  ...
  async optimize(): Promise<void> {
    ...
    for (const doc of allDocs) {
      if (currentShardSize >= optimalShardSize) {
        currentShardId++;
        currentShardSize = 0;
        if (!this.shards.has(currentShardId)) this.createNewShard();
      }
  ...
  async close(): Promise<void> {
    ...
    // Final sync save for any remaining dirty shards
    if (this.dirtyShards.size > 0) {
      for (const shardId of this.dirtyShards) {
        this.saveShardSync(shardId);
      }
      await this.saveMetadata();
      this.dirtyShards.clear();
    }
